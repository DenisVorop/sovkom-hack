...
class Cached():
    ...
    async def lazy_cache(self, id, cache=5, func=None, force_update_flag_function=None):
        print('lazy_cache')
        return await self.async_read_get_cached_or_redis(id ,
                        func,
                        cache_interval=cache, # 5, #0 #10 # кеширование в озу
                        redis_interval=60,#10, #60*60*24,
                        #force_update_flag_function = force_update_flag_function, 
                        conv='native',
                        return_json=json,
                        #only_redis=RAM_OPTIMIZED or False, # снижает количество памяти приложения, почти не влияет на производительность
                        only_cached=True,
                        ) 
