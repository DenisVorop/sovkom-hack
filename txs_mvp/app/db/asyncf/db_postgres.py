...
# https://stackoverflow.com/questions/43739123/best-way-to-insert-multiple-rows-with-asyncpg
    ...
    @staticmethod
    def to_sqlobj(func, obj, sql_fields=None): [func(obj), list(obj.values())] if sql_fields is None else  [func(obj), list(obj[sql_fields].values())] 


    @staticmethod
    def found_in_list(obj_list, name, id, select=True):
        found=[]
        [(found.append(a if select else id) if a[name] == id else None) for a in obj_list]
        return found